...
1. **新增 `host/`：屏幕捕获接收端（Rust + gpui）**
   不用摄像头，直接扫屏幕上一块区域的二维码流，用来做「隔离虚拟机 ↔ 宿主机」这种没网络、没共享剪贴板的场景。多显示器自动扫描、可框选区域、带传输进度 UI，文件落到 `~/Downloads` 并弹系统通知。协议映射、测试和已知限制见 [host/README.md](host/README.md)。
...
pnpm host:run      # 宿主机端：屏幕捕获接收（首次需授予「屏幕录制」权限）
